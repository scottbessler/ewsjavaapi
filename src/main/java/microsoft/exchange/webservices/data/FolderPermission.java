/**************************************************************************
 * copyright file="FolderPermission.java" company="Microsoft"
 *     Copyright (c) Microsoft Corporation.  All rights reserved.
 * 
 * Defines the FolderPermission.java.
 **************************************************************************/
...
public final class FolderPermission extends ComplexProperty implements
	...
	private static LazyMember<Map<FolderPermissionLevel, FolderPermission>> defaultPermissions =
	...
	private void AdjustPermissionLevel() {
		for (Entry<FolderPermissionLevel, FolderPermission> keyValuePair : defaultPermissions
				.getMember().entrySet()) {
			if (isEqualTo(keyValuePair.getValue())) {
				permissionLevel = keyValuePair.getKey();
				return;
			}
		}
	...
	public void setUserId(UserId value) {
		if (userId != null) {
			userId.removeChangeEvent(this);
		}

		if (this.canSetFieldValue(userId, value)) {
			userId = value;
			changed();
		}
		if (userId != null) {
			userId.addOnChangeEvent(this);
		}
	}
	 ...
	 * 
	 * @param value
	 *            the new can create items
	 */
	public void setCanCreateItems(boolean value) {
		if (this.canSetFieldValue(canCreateItems, value)) {
			canCreateItems = value;
			changed();
		}
	 ...
	 * 
	 * @param value
	 *            the new can create sub folders
	 */
	public void setCanCreateSubFolders(boolean value) {
		if (this.canSetFieldValue(canCreateSubFolders, value)) {
			canCreateSubFolders = value;
			changed();
		}
		AdjustPermissionLevel();
	}
	 ...
	 * 
	 * @param value
	 *            the new checks if is folder owner
	 */
	public void setIsFolderOwner(boolean value) {
		if (this.canSetFieldValue(isFolderOwner, value)) {
			isFolderOwner = value;
			changed();
		}
		AdjustPermissionLevel();
	...
	public void setIsFolderVisible(boolean value) {
		if (this.canSetFieldValue(isFolderVisible, value)) {
	...
	public void setIsFolderContact(boolean value) {
		if (this.canSetFieldValue(isFolderContact, value)) {
			isFolderContact = value;
			changed();
		}
		AdjustPermissionLevel();
	}
	 ...
	 * 
	 * @param value
	 *            the new edits the items
	 */
	public void setEditItems(PermissionScope value) {
		if (this.canSetFieldValue(editItems, value)) {
			editItems = value;
			changed();
		}
		AdjustPermissionLevel();
	 ...
	 * 
	 * @param value
	 *            the new delete items
	 */
	public void setDeleteItems(PermissionScope value) {
		if (this.canSetFieldValue(deleteItems, value)) {
			deleteItems = value;
			changed();
		}
		AdjustPermissionLevel();
	...
	public void setReadItems(FolderPermissionReadAccess value) {
		if (this.canSetFieldValue(readItems, value)) {
			readItems = value;
			changed();
		}
		AdjustPermissionLevel();
	...
	public void setPermissionLevel(FolderPermissionLevel value)
		...
		if (permissionLevel != value) {
			...
			AssignIndividualPermissions(defaultPermissions.getMember()
					.get(value));
			if (this.canSetFieldValue(permissionLevel, value)) {
	...
	protected boolean tryReadElementFromXml(EwsServiceXmlReader reader)
		...
		if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.UserId)) {
			userId = new UserId();
			userId.loadFromXml(reader, reader.getLocalName());
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.CanCreateItems)) {
			canCreateItems = reader.readValue(Boolean.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.CanCreateSubFolders)) {
			canCreateSubFolders = reader.readValue(Boolean.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.IsFolderOwner)) {
			isFolderOwner = reader.readValue(Boolean.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.IsFolderVisible)) {
			isFolderVisible = reader.readValue(Boolean.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.IsFolderContact)) {
			isFolderContact = reader.readValue(Boolean.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.EditItems)) {
			editItems = reader.readValue(PermissionScope.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.DeleteItems)) {
			deleteItems = reader.readValue(PermissionScope.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
				XmlElementNames.ReadItems)) {
			readItems = reader.readValue(FolderPermissionReadAccess.class);
			return true;
		} else if (reader.getLocalName().equalsIgnoreCase(
	...
	protected void writeToXml(EwsServiceXmlWriter writer,
		...
		writer.writeStartElement(getNamespace(), xmlElementName);
